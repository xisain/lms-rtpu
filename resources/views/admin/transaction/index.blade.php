                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                @if ($item->status == 'waiting_approval')
                                    <div class="flex space-x-2">
                                        <form action="{{ route('admin.transaction.approval', $item->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="status" value="approved">
                                            <button type="submit" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600 transition duration-150">
                                                Setujui
                                            </button>
                                        </form>
                                        <form action="{{ route('admin.transaction.approval', $item->id) }}" method="POST" class="inline-block">
                                            ...
                                            <button type="submit" class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 transition duration-150">
                                                Tolak
                                            </button>
                                        </form>
                                    </div>
    ...
    <div id="paymentProofModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            ...
            <div class="flex justify-end mt-4">
                <button onclick="closeModal()" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 transition duration-150">
                    Tutup
                </button>
            </div>
        </div>
