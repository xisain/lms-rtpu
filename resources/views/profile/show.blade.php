...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Enrolled Courses Accordion -->
        <div class="bg-white rounded-lg shadow-md">
            <button onclick="toggleAccordion('courses')"
                class="w-full px-6 py-4 flex items-center justify-between text-left hover:bg-gray-50 transition-colors">
                <div class="flex items-center space-x-3">
                    <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <svg id="courses-icon" class="w-5 h-5 text-gray-500 transform transition-transform" fill="none"
                    stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
                ...
                <div class="p-6">
                    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div
                            ...
                            <div
                                class="h-40 bg-gradient-to-br from-{{ $color['from'] }} to-{{ $color['to'] }} flex items-center justify-center">
                                @if($course->thumbnail)
                                <img src="{{ asset('storage/' . $course->thumbnail) }}"
                                    class="w-full h-full object-cover" alt="{{ $course->title }}">
                                @else
                            ...
                            <div class="p-4">
                                ...
                                <div class="mb-3">
                                    <div class="flex justify-between text-sm mb-1">
                                        ...
                                        <span class="text-{{ $color['text'] }} font-semibold">{{ $progress['percentage']
                                            }}%</span>
                                    </div>
                                    <div class="w-full bg-gray-200 rounded-full h-2">
                                        <div class="bg-{{ $color['text'] }} h-2 rounded-full"
                                ...
                                <div class="flex space-x-2">
                                    <a href="{{ route('course.show', $course->id) }}"
                                        class="flex-1 px-4 py-2 bg-{{ $color['text'] }} text-white rounded-lg hover:bg-{{ $color['hover'] }} transition-colors text-sm text-center">
                                        Review
                                    </a>
                                    <button
                                        class="px-4 py-2 border border-{{ $color['text'] }} text-{{ $color['text'] }} rounded-lg hover:bg-green-50 transition-colors text-sm">
                                        Certificate
                                    </button>
                                </div>
                                @else
                                ...
                                <a href="{{ route('course.show', $course->slugs) }}"
                                    class="block w-full px-4 py-2 bg-{{ $color['text'] }} text-white rounded-lg hover:bg-{{ $color['hover'] }} transition-colors text-sm text-center">
                                    Continue Learning
                                </a>
                                @endif
                            </div>
