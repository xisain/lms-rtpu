        ...
        <form id="courseForm" action="{{ route('dosen.course.update', $course->id) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded-lg p-6">
            ...
            <div class="mb-8 p-2 pt-6 rounded-lg border border-gray-300">
                ...
                <div id="materialsContainer" class="space-y-6">
                    ...
                    <div class="material-item border border-gray-300 rounded-lg p-4 bg-gray-50"
                        data-index="{{ $mIndex }}" data-material-id="{{ $material->id }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="materials[{{ $mIndex }}][nama_materi]" required value="{{ old("
                                materials.$mIndex.nama_materi", $material->nama_materi) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                        </div>
                        ...
                        <div class="mb-4 border-t pt-4">
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <label class="inline-flex items-center">
                                    <input type="checkbox"
                                        class="toggle-quiz w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                                        data-material-index="{{ $mIndex }}" {{ $material->quiz ? 'checked' : '' }}>
                                    <span class="ml-2 text-sm text-gray-600">Tambahkan Quiz</span>
                                </label>
                            </div>

                            <div class="quiz-section {{ $material->quiz ? '' : 'hidden' }}"
                                data-material-index="{{ $mIndex }}">
                                <div class="bg-white border border-gray-200 rounded-lg p-4 mb-4">
                                    @if($material->quiz)
                                    <input type="hidden" name="materials[{{ $mIndex }}][quiz][id]"
                                        value="{{ $material->quiz->id }}">
                                    @endif
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="materials[{{ $mIndex }}][quiz][judul_quiz]"
                                            value="{{ old(" materials.$mIndex.quiz.judul_quiz",
                                            $material->quiz->judul_quiz ?? '') }}"
                                        class="quiz-input w-full px-3 py-2 border border-gray-300 rounded focus:ring-2
                                        focus:ring-blue-500"
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="questions-container space-y-4" data-material-index="{{ $mIndex }}">
                                            ...
                                            <div class="question-item bg-gray-50 border border-gray-200 rounded p-3"
                                                ...
                                                <input type="hidden"
                                                    name="materials[{{ $mIndex }}][quiz][questions][{{ $qIndex }}][id]"
                                                    value="{{ $question->id }}">

                                                <div class="flex justify-between items-start mb-2">
                                                    <label class="block text-sm font-medium text-gray-700">Pertanyaan
                                                        #{{ $qIndex + 1 }}</label>
                                                    <button type="button"
                                                        class="remove-question text-red-600 hover:text-red-800">
                                                ...
                                                <div class="mb-3">
                                                    <input type="text"
                                                        name="materials[{{ $mIndex }}][quiz][questions][{{ $qIndex }}][pertanyaan]"
                                                        value="{{ old("
                                                        materials.$mIndex.quiz.questions.$qIndex.pertanyaan",
                                                        $question->pertanyaan) }}"
                                                    class="quiz-input w-full px-3 py-2 text-sm border border-gray-300
                                                    rounded"
                                                    placeholder="Tulis pertanyaan di sini...">
                                                </div>

                                                <div class="space-y-2">
                                                    <p class="text-sm font-medium text-gray-700 mb-1">Pilihan Jawaban:
                                                    </p>
                                                    <div class="grid grid-cols-1 gap-2">
                                                        @foreach ($question->options as $oIndex => $option)
                                                        <div class="flex items-center gap-2">
                                                            <input type="hidden"
                                                                name="materials[{{ $mIndex }}][quiz][questions][{{ $qIndex }}][options_ids][]"
                                                                value="{{ $option->id }}">
                                                            <input type="radio"
                                                            ...
                                                            <input type="text"
                                                                ...
                                                                value="{{ old("
                                                                materials.$mIndex.quiz.questions.$qIndex.options.$oIndex",
                                                                $option->teks_pilihan) }}"
                                                            class="quiz-input flex-1 px-3 py-1.5 text-sm border
                                        ...
                                        <button type="button"
                                            class="add-question mt-3 text-sm bg-gray-100 hover:bg-gray-200 text-gray-600 py-2 px-4 rounded"
                                            data-material-index="{{ $mIndex }}">
                                            + Tambah Pertanyaan
                                        </button>
                                    </div>
                        ...
                        <div class="mb-3">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <button type="button"
                                    class="add-submaterial text-white hover:bg-[#0f5757] font-small bg-[#009999] rounded-[10px] border border-gray-300 px-3 py-2 shadow-lg transition"
                                    data-material-index="{{ $mIndex }}">
                                    + Tambah Submateri
                                </button>
                            </div>
                            <div class="submaterials-container space-y-3" data-material-index="{{ $mIndex }}">
                                ...
                                <div class="submaterial-item border border-gray-200 rounded-lg p-3 bg-white"
                                    data-submaterial-index="{{ $sIndex }}" data-submaterial-id="{{ $sub->id }}">
                                    <input type="hidden"
                                        name="materials[{{ $mIndex }}][submaterials][{{ $sIndex }}][id]"
                                        value="{{ $sub->id }}">

                                    <div class="flex justify-between items-center mb-3">
                                        <h4 class="text-sm font-semibold text-gray-700">Submateri #{{ $sIndex + 1 }}
                                        </h4>
                                        <button type="button"
                                            class="remove-submaterial text-red-600 hover:text-red-800">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                                        <div>
                                            <label class="block text-xs font-medium text-gray-700 mb-1">Nama Submateri
                                                *</label>
                                            <input type="text"
                                                name="materials[{{ $mIndex }}][submaterials][{{ $sIndex }}][nama_submateri]"
                                                required value="{{ old("
                                                materials.$mIndex.submaterials.$sIndex.nama_submateri",
                                                $sub->nama_submateri) }}"
                                            class="w-full px-3 py-2 text-sm border border-gray-300 rounded focus:ring-2
                                            focus:ring-blue-500">
                                        </div>
                                        <div>
                                            ...
                                            <select name="materials[{{ $mIndex }}][submaterials][{{ $sIndex }}][type]"
                                                ...
                                                <option value="text" {{ $sub->type == 'text' ? 'selected' : '' }}>Text
                                                </option>
                                                <option value="video" {{ $sub->type == 'video' ? 'selected' : ''
                                                    }}>Video (YouTube)</option>
                                                <option value="pdf" {{ $sub->type == 'pdf' ? 'selected' : '' }}>PDF
                                                </option>
                                            </select>
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-gray-700 mb-1">Sembunyikan Materi</label>
                                            <label class="relative inline-flex items-center cursor-pointer">
                                                <input type="checkbox"
                                                       name="materials[{{ $mIndex }}][submaterials][{{ $sIndex }}][hidden]"
                                                       value="1"
                                                       {{ $sub->hidden ? 'checked' : '' }}
                                                       class="sr-only peer">
                                    ...
                                    <div class="sub-content">
                                        @if ($sub->type === 'text')
                                        <label class="block text-xs font-medium text-gray-700 mb-1">Isi Materi (Teks)
                                            *</label>
                                        <input id="trix-{{ $mIndex }}-{{ $sIndex }}" type="hidden"
                                            name="materials[{{ $mIndex }}][submaterials][{{ $sIndex }}][isi_materi]"
                                            value="{{ old(" materials.$mIndex.submaterials.$sIndex.isi_materi",
                                            $sub->isi_materi) }}"
                                        required>
                                        ...
                                        <label class="block text-xs font-medium text-gray-700 mb-1">Link YouTube
                                            *</label>
                                        <input type="text"
                                            name="materials[{{ $mIndex }}][submaterials][{{ $sIndex }}][isi_materi]"
                                            required value="{{ old(" materials.$mIndex.submaterials.$sIndex.isi_materi",
                                            $sub->isi_materi) }}"
                                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded focus:ring-2
                                        focus:ring-blue-500"
                                        placeholder="https://www.youtube.com/watch?v=...">
                                        @elseif ($sub->type === 'pdf')
                                        <label class="block text-xs font-medium text-gray-700 mb-1">Upload File
                                            PDF</label>
                                        <p class="text-xs text-gray-500 mb-1">File saat ini: {{
                                            basename($sub->isi_materi) }}</p>
                                        <input type="file"
                                            name="materials[{{ $mIndex }}][submaterials][{{ $sIndex }}][isi_materi]"
                                            accept="application/pdf"
                                            class="w-full px-3 py-2 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500">
                                        <!-- IMPORTANT: Hidden input to preserve existing PDF path -->
                                        <input type="hidden"
                                            name="materials[{{ $mIndex }}][submaterials][{{ $sIndex }}][isi_materi]"
                                            value="{{ $sub->isi_materi }}">
                                        @endif
                                    </div>
                                </div>
                                @endforeach
            ...
            <div class="submaterial-item border border-gray-200 rounded-lg p-3 bg-white" data-submaterial-index="${submaterialIndex}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                    ...
                    <div>
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox"
                                   name="materials[${materialIndex}][submaterials][${submaterialIndex}][hidden]"
                                   value="1"
                                   class="sr-only peer">
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                            <span class="ml-3 text-sm font-medium text-gray-700">Hidden</span>
